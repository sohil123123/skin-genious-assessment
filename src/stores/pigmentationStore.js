...
export const usePigmentationStore = defineStore('pigmentation', {
  ...
  actions: {
    ...
    resetState() {
      ...
      this.redFlags = []
      this.formData = {
        fitz: '', comp: '', mel: '', ery: '', woods: '', depth: '',
        initials: '', mrn: '', age: '', sex: '',
        dist: '', dur: '', onset: '', prog: '',
        triggers: [], hqHistory: false, priorTx: '', meds: '', notes: ''
      }
      this.diagnosis = null
      this.dermoscopyFindings = null
      this.demoDxAsked = false
      this.lastPlan = null
      this.reviewState = { decision: null, notes: '', reviewer: 'Dr. A. Mehra', finalized: false, ts: null }
      this.goals = []
      this.reassessment = null
    },
    
    async callClaude({ system, content, max_tokens = 1000, temperature = 0 }) {
      if (this.demoMode) {
      ...
      if (!response.ok) {
        let msg = `Anthropic API error: ${response.statusText} (${response.status})`
        if (response.status === 401) msg += ' — check your API key.'
        else if (response.status === 429) msg += ' — rate limited, wait a moment and retry.'
        else if (response.status === 400) msg += ' — bad request (an attached image may be too large).'
        throw new Error(msg)
      }
    ...
    async demoResponse({ system, content }) {
      ...
      if (/diagnostic assistant/.test(system)) {
        ...
        const hasHQ = /HQ use:\s*YES/i.test(textContent) || 
        ...
        const dermoProvided = /dermoscopy image\(s\) are attached/i.test(textContent) || 
      ...
      if (/reassessment assistant/.test(system)) {
        ...
        const re = /Metric:\s*(.*?)\s*\|\s*Baseline:\s*(.*?)\s*\|\s*Target:\s*(.*?)\s*\|\s*Timeframe:\s*(.*?)\s*\|\s*Current:\s*(.*)$/
        ...
        const rec = traj === 'plateaued'
          ? { action: 'escalate', detail: 'Little movement — reinforce adherence and consider stepping up a tier or adding a gentle procedure after priming.' }
          : { action: 'continue', detail: 'Trajectory is acceptable — continue the regimen and photoprotection, and reassess at the next interval.' }
          
        const out = {
          overall: { trajectory: traj, summary: (traj === 'improving' ? 'On track against the goals set at assessment.' : traj === 'mixed' ? 'Partial progress — some goals moving, others not yet.' : 'Limited change so far this interval.') },
          goals: goals.length ? goals : [{ metric: '(no goals entered)', status: 'unknown', delta: '—', comment: 'Add goals and current values to reassess.' }],
          recommendation: rec,
          diagnosis_reexamine: { needed: false, reason: '' },
          patient_summary: 'This is a demo reassessment: progress is compared against the targets set at the first visit; the plan continues with any adjustments noted above.',
    ...
    buildImageContext() {
      ...
      lines.push('', 'Produce the objective read, provisional impression, and tailored history questions as the specified JSON.')
      return lines.join('\n')
    },
    ...
    buildBaseBlock() {
      ...
      const rows = [
        ...
        line('Fitzpatrick (AI-estimated from white-light capture, clinician-confirmed)', this.formData.fitz),
        '',
        'PRESENTATION:',
        ...
        line('Analyser captures attached', this.attachedImages.length ? `${this.attachedImages.length} image(s) below` : 'none'),
        '',
        'SAFETY SCREEN:',
    ...
    buildDiagnosisInput() {
      ...
      if (this.dermoscopyImages.length) {
        ...
        if (this.dermoscopyFindings) {
          ...
          if (f.observed_structures?.length) dermo += ` Structures: ${f.observed_structures.join(', ')}.`
          if (f.feature_checks?.length) dermo += ' Feature checks: ' + f.feature_checks.map((c) => `${c.feature}=${c.status}`).join('; ') + '.'
          if (f.suggests) dermo += ` Dermoscopic impression: ${f.suggests}`
        }
        dermo += ' Incorporate these dermoscopy findings, set needs_dermoscopy=false, and commit to your best-supported diagnosis.'
      } else {
    ...
    async extractDermoscopy() {
      ...
      const systemPrompt = DERMOSCOPY_PROMPT + (lookFor.length ? `\n\nSpecific features requested: ${lookFor.join(', ')}.` : '')
      
      const content = [{ type: 'text', text: 'Extract dermoscopic structures from these attached dermoscopy captures.' }]
      this.dermoscopyImages.forEach((img) => {
        content.push({ type: 'image', source: { type: 'base64', media_type: img.mediaType, data: img.base64 } })
      })
    ...
    buildReassessInput() {
      ...
      this.goals.forEach((g, i) => {
        lines.push(`${i + 1}. Metric: ${g.metric} | Baseline: ${g.baseline || '—'} | Target: ${g.target || '—'} | Timeframe: ${g.timeframe || '—'} | Current: ${g.current || '(not entered)'}`)
      })
      
      lines.push('')
      if (this.reassessImages.length) {
        lines.push(`Follow-up captures (${this.reassessImages.length}) are attached below — re-read them to inform current status where a value was not entered.`)
      } else {
