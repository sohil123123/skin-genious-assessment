...
export const SESSION_WINDOWS_V39 = Object.freeze({
  single: [60, 75],
  express: [35, 45],
  multiple: [60, 75],
})
export const STEP_RULES_V39 = Object.freeze({
  ...OPERATIONAL_DURATIONS_V39,
  ...CLINIC_STEP_DURATION_RULES_V3_4,
  carbon_lotion_application_and_dry: {
    min_minutes: 3,
    max_minutes: 3,
    source: 'clinic_v3.4_carbon_substep',
  },
...
export function selectedConcernsV39(value) {
  let current = value
  for (let i = 0; i < 3 && !Array.isArray(current) && current; i++)
    current = current.parameters_with_abnormal_scores
  if (!Array.isArray(current))
    throw new Error('The selected concern list is missing. Please return to concern selection.')
...
export function validateWorkflowPlanV39(output, mode, selected = [], context = {}) {
  const errors = [],
    sessions = output?.treatment_plan?.treatments,
    window = SESSION_WINDOWS_V39[mode]
  if (!window) return { valid: false, errors: ['Unknown product type.'] }
  if (!Array.isArray(sessions) || !sessions.length)
    return { valid: false, errors: ['No treatment sessions returned.'] }
  const outline = output.treatment_plan.course_outline
  const hasSteps = (x) =>
    x && typeof x === 'object' && (Object.hasOwn(x, 'steps') || Object.values(x).some(hasSteps))
  if (hasSteps(outline) || hasSteps(output.recommended_full_plan))
    errors.push('Future course outlines must not contain detailed steps.')
  const start = context.next_session_number ?? 1
  const total = mode === 'multiple' ? output.treatment_plan.estimated_sessions : 1
  if (mode === 'multiple' && (!Number.isInteger(total) || total < 5 || total > 8))
    errors.push('Course estimate must be 5–8 sessions.')
  if (context.estimated_sessions != null && total !== context.estimated_sessions)
    errors.push('A continuation must preserve the current estimated course length.')
  if (
    mode === 'multiple' &&
    start > 1 &&
    (start % 2 !== 1 ||
      context.reassessed_after_session !== start - 1 ||
      !context.reassessment_scan_id)
  )
    errors.push('Continuation requires reassessment at the end of the preceding two-session block.')
  const count = mode === 'multiple' ? Math.min(2, total - start + 1) : 1
  if (count < 1 || sessions.length !== count)
    errors.push(
      'Return only the current released block, including a one-session final block when needed.',
    )
  ...
  for (const [index, session] of sessions.entries()) {
    ...
    if (!Array.isArray(steps) || !steps.length) {
      errors.push(`${path}: missing steps.`)
      continue
    }
    let sum = 0
    for (const [i, s] of steps.entries()) {
      ...
      if (typeof s.duration !== 'number' || !Number.isFinite(s.duration) || s.duration <= 0) {
        errors.push(`${path}, step ${i + 1}: duration must be a positive number.`)
        continue
      }
      if (
        rule?.minutes_per_ingredient &&
        (typeof s.infusion_ingredient !== 'string' || !s.infusion_ingredient.trim())
      )
        errors.push(`${path}, step ${i + 1}: name the single infusion ingredient.`)
      sum += s.duration
      if (rule && (s.duration < rule.min_minutes || s.duration > rule.max_minutes))
        errors.push(
          `${path}, ${s.modality_id}: duration outside ${rule.min_minutes}–${rule.max_minutes} minutes.`,
        )
    }
    const ids = steps.map((s) => s.modality_id)
    if (
      ids[0] !== 'cleanse_and_prepare' ||
      ids.at(-1) !== 'final_serum_moisturizer_sunscreen' ||
      ids.filter((x) => x === 'final_serum_moisturizer_sunscreen').length !== 1
    )
      errors.push(`${path}: required opening/final step missing or duplicated.`)
    if (!ids.includes('lymphatic_drainage'))
      errors.push(`${path}: mandatory lymphatic drainage missing.`)
    const carbon =
      ids.includes('carbon_lotion_application_and_dry') || ids.includes('carbon_qswitch_laser')
    if (
      carbon &&
      (ids.includes('carbon_facial') ||
        ids.filter((x) => x === 'carbon_lotion_application_and_dry').length !== 1 ||
        ids.filter((x) => x === 'carbon_qswitch_laser').length !== 1 ||
        ids.indexOf('carbon_lotion_application_and_dry') > ids.indexOf('carbon_qswitch_laser'))
    )
      errors.push(`${path}: invalid/duplicated carbon sequence.`)
    if (sum < window[0] || sum > window[1])
      errors.push(`${path}: ${sum} minutes does not meet ${window[0]}–${window[1]}.`)
    if (
      session.treatment_time !== sum ||
      session.step_duration_total !== sum ||
      session.timing_validation?.calculated_from_steps !== sum ||
      session.timing_validation?.matches_treatment_time !== true
    )
      errors.push(`${path}: declared duration does not equal actual step sum.`)
    if (
      !Number.isInteger(session.day_offset) ||
      !Number.isInteger(session.gap_days_from_previous) ||
      (number === 1 && (session.day_offset !== 0 || session.gap_days_from_previous !== 0)) ||
      (number > 1 &&
        (session.day_offset <= previousDay ||
          session.gap_days_from_previous !== session.day_offset - previousDay ||
          typeof session.spacing_reason !== 'string' ||
          !session.spacing_reason.trim()))
    )
      errors.push(`${path}: inconsistent course schedule.`)
    if (
      typeof session.week !== 'number' ||
      Math.abs(session.week - (1 + session.day_offset / 7)) > 1e-6
    )
      errors.push(`${path}: week does not match day offset.`)
    previousDay = session.day_offset
    for (const c of session.concerns_addressed ?? [])
      if (c.parameter_id) addressed.add(c.parameter_id)
  }
  const exceptions = output.treatment_plan.primary_concern_exceptions ?? []
  for (const c of selected.filter((x) => x.is_primary_concern === true))
    if (
      c.parameter_id &&
      !addressed.has(c.parameter_id) &&
      !exceptions.some(
        (x) => x.parameter_id === c.parameter_id && typeof x.reason === 'string' && x.reason.trim(),
      )
    )
      errors.push(`Selected primary ${c.parameter_id} was omitted without an explanation.`)
  return {
    valid: errors.length === 0,
    errors,
    scope:
      'Structural/timing/selection/schedule-arithmetic validation; clinical eligibility and interval appropriateness remain governed by the existing supplied planning constraints.',
  }
}
...
export function mergeCourseBlockV39(existing, incoming) {
  ...
  for (const next of incoming.treatment_plan.treatments) {
    ...
    if (prior)
      throw new Error(
        `Session ${next.session_number} already exists; do not regenerate or overwrite it.`,
      )
  ...
  out.treatment_plan.course_outline =
    incoming.treatment_plan.course_outline ?? out.treatment_plan.course_outline
  return out
}
...
export function planningEvidenceV39(value) {
  ...
  return Object.fromEntries(
    Object.entries(value)
      .filter(
        ([key]) =>
          !/(confidence|reliability|uncertainty)/i.test(key) &&
          ![
            'data_quality',
...
export function normalizeWorkflowPlanV392(value, mode) {
  ...
  const number = (x) =>
    typeof x === 'string' && /^\d+(?:\.\d+)?(?:\s*(?:mins?|minutes?))?$/i.test(x.trim())
      ? Number.parseFloat(x)
      : x
  if (plan.treatment_plan.estimated_sessions != null)
    plan.treatment_plan.estimated_sessions = number(plan.treatment_plan.estimated_sessions)
  for (const session of plan.treatment_plan.treatments) {
    for (const key of ['session_number', 'week', 'day_offset', 'gap_days_from_previous'])
      if (session[key] != null) session[key] = number(session[key])
    if (mode !== 'multiple') {
      session.session_number ??= 1
      session.day_offset ??= 0
      session.gap_days_from_previous ??= 0
      session.week ??= 1
    }
    if (!Array.isArray(session.steps)) continue
    session.steps.forEach((step, i) => {
      step.duration = number(step.duration)
    ...
    if (session.steps.every((x) => typeof x.duration === 'number' && Number.isFinite(x.duration))) {
      ...
      session.treatment_time = total
      session.step_duration_total = total
      session.timing_validation = {
        ...session.timing_validation,
