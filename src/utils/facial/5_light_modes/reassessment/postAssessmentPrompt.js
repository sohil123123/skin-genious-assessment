...
const reassessment_json_structure = {
  reassessment: {
    skin_type: {
      ...
      before_image:
        'Return the image number (1-5) from the uploaded face scan images that best represents the area analyzed for this parameter from previously generated baseline (before-treatment) results stored in this conversation memory.',
      post_treatment_score_or_label:
        '<Enter FINAL patient-facing post-treatment Skin Type shown in report>',
      post_treatment_image:
        'Return the image number (1-5) from the uploaded face scan images that best represents the area analyzed for this parameter from newly analyzed post-treatment results.',
    ...
    barrier_health_sensitivity: {
      parameter_name: 'Barrier Health + Sensitivity',
      before_treatment_score_or_label: '<Enter Barrier Health + Sensitivity Score>',
      before_image:
        'Return the image number (1-5) from the uploaded face scan images that best represents the area analyzed for this parameter from previously generated baseline (before-treatment) results stored in this conversation memory.',
      post_treatment_score_or_label:
        '<Enter FINAL patient-facing post-treatment Barrier Health + Sensitivity Score shown in report>',
      post_treatment_image:
        'Return the image number (1-5) from the uploaded face scan images that best represents the area analyzed for this parameter from newly analyzed post-treatment results.',
      result: '<improved or stable>',
    ...
    textural_radiance_index: {
      ...
      before_treatment_score_or_label: '<Enter Textural Radiance Range>',
      before_image:
        'Return the image number (1-5) from the uploaded face scan images that best represents the area analyzed for this parameter from previously generated baseline (before-treatment) results stored in this conversation memory.',
      post_treatment_score_or_label:
        '<Enter FINAL patient-facing post-treatment Textural Radiance Range shown in report>',
      post_treatment_image:
        'Return the image number (1-5) from the uploaded face scan images that best represents the area analyzed for this parameter from newly analyzed post-treatment results.',
      result: '<improved or stable>',
      score_semantics: 'severity',
...
Do not change the underlying baseline interpretation methods, scales, thresholds, parameter meanings, or polarity rules.
...
The current pipeline uses 5 facial scan images captured under these modes:
1. Red
...
EXPLICIT COMPARISON RULES FOR INTERNAL RAW COMPARISON:
  ...
  if distance_to_target:
    compare improvement as reduced distance from the clinically ideal balance zone defined by the original scoring rubric for that parameter
...
- periorbital_health
- lip_pigmentation
- superficial_wrinkles_scoring (dehydration microlines only; fixed lines do not change same-day)
...
- a number 1-5 → post_treatment_image must be that exact same number
