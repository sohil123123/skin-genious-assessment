<template>
  <section class="stage is-active">
    <div class="stage-head">
      ...
      <p>Confirm the final working diagnosis. The AI differential is proposed from the clinical image read and history context. Clinician verification is mandatory before generating a treatment plan.</p>
    </div>

    <!-- VALIDATION ERROR -->
    <div class="card bg-red-1 q-mb-md" v-if="validationError" id="dxValidate" style="border: 1px solid var(--erythema)">
      <div class="err-box">
    ...
    <div v-if="store.diagnosis && !store.isLoading" id="dxOutput">
      ...
      <div v-if="store.diagnosis.data?.needs_dermoscopy" class="dermo-req">
        ...
        <div style="font-size:13px;color:#4a3d6b;margin-bottom:12px">
          {{ store.diagnosis.data.dermoscopy_request?.reason || 'The captures and history alone don\'t give enough confidence to commit to a diagnosis.' }}
        </div>
        
        <div style="font-size:12.5px;color:#4a3d6b;margin-bottom:16px">
          ↓ Add the dermoscopy image(s) below to extract the findings, then update the diagnosis. No dermoscope to hand?
          <a href="#" @click.prevent="proceedWithoutDermoscopy" style="color:var(--violet); font-weight:600; text-decoration: underline">
            Proceed on the provisional read
          </a> — records that dermoscopy was advised.
        </div>
        ...
        <div class="viewer-card" id="dermoCard" style="background:#261f30; margin-bottom: 12px">
          <div class="viewer-head">
            ...
            <span class="t" style="color:#a89fb6">{{ store.dermoscopyImages.length }} attached</span>
          </div>
          ...
          <input type="file" ref="dermoInput" accept="image/*" multiple hidden @change="onDermoFileChange">
        ...
        <div class="card" v-if="store.dermoscopyFindings" style="background:#f9f8fc; border-color:#c9beea; margin-bottom:12px">
          ...
          <div class="airead-line" v-if="store.dermoscopyFindings.pattern_summary">
            ...
            <span class="v"><b>{{ store.dermoscopyFindings.pattern_summary }}</b></span>
          </div>
          ...
          <div class="airead-line" v-if="store.dermoscopyFindings.feature_checks?.length">
            ...
            <span class="v">
              <div v-for="(fc, fIdx) in store.dermoscopyFindings.feature_checks" :key="fIdx" style="margin-bottom:4px">
          ...
          <div class="airead-line" v-if="store.dermoscopyFindings.suggests">
            ...
            <span class="v" style="color:var(--violet)">{{ store.dermoscopyFindings.suggests }}</span>
          </div>

          <div class="airead-line" v-if="store.dermoscopyFindings.quality_caveat">
            <span class="k">Quality Note</span>
            <span class="v"><small>{{ store.dermoscopyFindings.quality_caveat }}</small></span>
          </div>

          <div class="airead-line" v-if="store.dermoscopyFindings.red_flags?.present" style="color:var(--erythema)">
            <span class="k" style="color:var(--erythema)">RED FLAGS</span>
            <span class="v" style="color:var(--erythema)"><b>{{ store.dermoscopyFindings.red_flags.items?.join('; ') }}</b></span>
          </div>

          <button class="btn btn-primary btn-block" style="margin-top:12px" @click="updateDxWithDermoscopy">
      ...
      <div v-else>
        ...
        <div class="pblock">
          ...
          <div v-if="store.diagnosis.data.differential?.alternatives?.length">
            <div v-for="(alt, aIdx) in store.diagnosis.data.differential.alternatives" :key="aIdx" class="dx-alt">
              <div class="l">
                <b>{{ alt.dx }}</b>
        ...
        <div class="pblock">
          <div class="twin">
            <div :class="['vbox', getDepthClass(store.diagnosis.data.depth_assessment?.verdict)]">
              ...
              <div class="hd">{{ cap(store.diagnosis.data.depth_assessment?.verdict || store.formData.depth || 'uncertain') }}</div>
              <div class="ds">
                {{ store.diagnosis.data.depth_assessment?.prognosis || store.diagnosis.data.depth_assessment?.basis || '' }}
              </div>
            </div>
            
            <div :class="['vbox', getCompClass(store.diagnosis.data.composition_assessment?.dominant)]">
              <div class="lab">Composition</div>
              <div class="hd">{{ cap(store.diagnosis.data.composition_assessment?.dominant || store.formData.comp || '—') }}</div>
        ...
        <div class="pblock" v-if="store.diagnosis.data.scores?.length">
          ...
          <div class="scoregrid">
            <div class="scorecard" v-for="(s, sIdx) in store.diagnosis.data.scores" :key="sIdx">
              <div class="sl">
                {{ s.name }} <span style="text-transform:none;font-weight:400" v-if="s.scale">({{ s.scale }})</span>
              </div>
              <div class="sv">{{ s.value !== null && s.value !== '' ? s.value : '—' }}</div>
              <div class="si">{{ s.interpretation }}</div>
            </div>
          </div>
          <p class="note" style="margin-top:8px;font-size:11.5px">Scores are AI estimates from the images/history for clinician confirmation.</p>
        </div>
        ...
        <div class="pblock" v-if="store.diagnosis.data.red_flags?.present">
          <div class="redflag">
            ...
            <div class="bd">
              ...
              <div style="margin-top:4px"><b>Action advised:</b> {{ store.diagnosis.data.red_flags.action }}</div>
            </div>
          </div>
        </div>
        ...
        <div class="pblock" id="confirmDxBlock">
          ...
          <p class="note" style="margin-bottom:11px">The plan is built on whatever you confirm here. Change it if your clinical judgement differs from the AI.</p>
          ...
          <button class="btn btn-primary" style="margin-top:12px" @click="confirmDxSelection" v-if="!store.diagnosis.confirmedDx">
            ✓ Confirm working diagnosis
          </button>
          
          <div class="stamp ok show" v-if="store.diagnosis.confirmedDx" id="dxStamp" style="margin-top:12px">
...
const dxSelectOptions = computed(() => {
  ...
  const primary = store.diagnosis.data.differential?.primary?.dx
  const alternatives = store.diagnosis.data.differential?.alternatives || []
  
  const list = []
  if (primary) {
    list.push({ value: primary, label: primary })
  }
  alternatives.forEach(alt => {
    if (alt.dx && alt.dx !== primary) {
      list.push({ value: alt.dx, label: alt.dx })
    }
  })
...
const confirmDxSelection = () => {
  const chosen = selectedDxChoice.value === '__custom__' ? customDxValue.value.trim() : selectedDxChoice.value
  if (!chosen) {
